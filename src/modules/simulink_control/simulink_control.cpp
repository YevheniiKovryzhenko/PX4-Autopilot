...
int SimulinkControl::print_status()
{
	PX4_INFO("Running");
...
{
	_task_id = px4_task_spawn_cmd("simulink_control",
				      SCHED_DEFAULT,
...
SimulinkControl *SimulinkControl::instantiate(int argc, char *argv[])
{
	...
	SimulinkControl *instance = new SimulinkControl(example_param, example_flag);
...
SimulinkControl::SimulinkControl(int example_param, bool example_flag)
...
void SimulinkControl::update_simulink_io(void)
{
	update_simulink_inputs();
}

void SimulinkControl::update_simulink_inputs(void)
{
...
bool SimulinkControl::update_sticks(int input_source_opt, sticks_ind stick, float& stick_val)
{
...
// bool SimulinkControl::update_man_wing_angle(int input_source_opt, float& wing_cmd)
// {
...
bool SimulinkControl::update_distance_sensor(void)
{
	return _distance_sensor_sub.update(&dist);
}

bool SimulinkControl::update_airspeed(void)
{
	return _airspeed_sub.update(&airspeed);
}

bool SimulinkControl::update_adc(void)
{
	return _adc_report_sub.update(&adc);
}

bool SimulinkControl::check_ground_contact(void) // this is a quick work-around the weird land-detector logic
{
...
bool SimulinkControl::check_armed(bool &armed, int input_src_opt)
{
...
void SimulinkControl::printf_debug_array(debug_array_s &array)
{
...
bool SimulinkControl::update_mode_autonomous(control_level &current_mode, bool armed)
{
...
bool SimulinkControl::update_mode(float &current_mode, int input_source_opt, bool &use_raw_mode_switch)
{
...
bool SimulinkControl::update_aux_stick(int input_source_opt, float &value, uint8_t stick_ind)
{
...
bool SimulinkControl::update_control_inputs(float in_vec[CONTROL_VEC_SIZE])
{
	bool need_update = false;
...
void
SimulinkControl::publish_inbound_sim_data(void)
{
	...
	if (need_2_pub)
	{
		...
		// simulink_inboud_data.fill_buffer(global_pos.vel_e_m_s); //29
		// simulink_inboud_data.fill_buffer(global_pos.vel_d_m_s); //30
		// simulink_inboud_data.fill_buffer(global_pos.vel_n_m_s); //31
...
void SimulinkControl::parameters_update(bool force)
{
...
int SimulinkControl::print_usage(const char *reason)
{
	...
	PRINT_MODULE_DESCRIPTION(
		R"DESCR_STR(
### Description
Section that describes the provided simulink_control module functionality.

This is a template for a simulink_control module running as a task in the background with start/stop/status functionality.

### Implementation
Section describing the high-level implementation of this simulink_control module.

### Examples
CLI usage example:
$ simulink_control start -f -p 42

)DESCR_STR");

	PRINT_MODULE_USAGE_NAME("simulink_control", "custom");
